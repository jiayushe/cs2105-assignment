...
caseA() {
  ...
  let i--
	nreqs=$(PYTHONPATH="$d1" "$py36" -c "from cases1 import *; print(len(casegrps[$i])//2)")
	#echo "$nreqs"
  ...
  for p0 in $(genRandPorts); do
    ...
    #
    rm -f "$tmpdir/c0-err"
    PYTHONPATH="$d1" "$py36" -c "import client" "$port" "$i" 0 0 2> "$tmpdir/c0-err"
    rtn="$?"
    if [[ "$rtn" == 1 ]]; then
      port=""
  ...
  for j in $( seq 1 $((nreqs-1)) ); do
    ...
    clog="$tmpdir/c$j-err"
    rm -f "$clog"
    PYTHONPATH="$d1" "$py36" -c "import client" "$port" "$i" "$j" 0 2> "$clog"
    rtn="$?"
...
caseBCD() {
  ...
  for p0 in $(genRandPorts); do
    ...
    #
    rm -f "$tmpdir/c-err"
    PYTHONPATH="$d1" "$py36" -c "import client${cver}" "$port" "$i" -1 0 2> "$tmpdir/c-err"
    rtn="$?"
    if [[ "$rtn" == 1 ]]; then
      port=""
      kill -9 $spid 2>/dev/null
